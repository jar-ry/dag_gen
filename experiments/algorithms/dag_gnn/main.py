...
def main(opt, metrics_callback=None, plotting_callback=None):
    ...
    train_data_np = (
        train_data.dataset.unsqueeze(2).detach().cpu().numpy()
    )  # shape (num_samples, num_nodes, node_dim)
    num_nodes = train_data_np.shape[1]

    initial_adj = np.ones((num_nodes, num_nodes), dtype=np.double) - np.eye(
        num_nodes, dtype=np.double
    )

    if opt.pns:
        initial_adj = pns_(
            initial_adj, train_data, test_data, opt.num_neighbors, opt.pns_thresh
        )

    # apply DAG-GNN
    w_adj, rel_rec, rel_send, encoder, decoder, train_loader, flag_max_iter = dag_gnn(
        opt, train_data_np, gt_dag, initial_adj
    )
    ...
    if opt.cam_pruning:
        ...
        # assert that cam_pruning is not adding edges
        assert (adj >= new_adj).all()
        adj = new_adj
    ...
    #                  metrics={"train_score": score_train, "val_score": score_valid, "shd": shd,
...
def run_dag_gnn(config):
    ...
    parser.add_argument(
        "--data-path", type=str, default=config["data_path"], help="Path to data files"
    )
    parser.add_argument("--i-dataset", type=str, default=1, help="dataset index")
    parser.add_argument(
        "--exp-path",
        type=str,
    ...
    parser.add_argument(
        "--optimizer", type=str, default="Adam", help="the choice of optimizer used"
    )
    parser.add_argument(
        "--graph_threshold",
        type=float,
    ...
    parser.add_argument(
        "--use_A_connect_loss", type=int, default=0, help="flag to use A connect loss"
    )
    parser.add_argument(
