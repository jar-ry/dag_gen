...
def main(opt, metrics_callback= None, plotting_callback=None):
    ...
    train_data_np = train_data.dataset.unsqueeze(2).detach(
    ).cpu().numpy()  # shape (num_samples, num_nodes, node_dim)
    num_nodes = train_data_np.shape[1]

    initial_adj = np.ones((num_nodes, num_nodes),
                          dtype=np.double) - np.eye(num_nodes, dtype=np.double)

    if opt.pns:
        initial_adj = pns_(initial_adj, train_data, test_data,
                           opt.num_neighbors, opt.pns_thresh)
    ...
    # cam_pruning?
    if opt.cam_pruning:
        new_adj = cam_pruning_(adj, train_data, test_data,
                               opt.cutoff, opt.exp_path)
