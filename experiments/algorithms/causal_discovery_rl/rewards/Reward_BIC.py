...
class get_Reward(object):
    ...
    def calculate_reward_single_graph(self, graph_batch, lambda1, lambda2):
        ...
        for i in range(self.maxlen):
            ...
            graph_to_int.append(
                self.baseint * i + np.int("".join([str(ad) for ad in tt]), 2)
            )
        ...
        if graph_batch_to_tuple in self.d:
            ...
            return (
                self.penalized_score(score_cyc, lambda1, lambda2),
                score_cyc[0],
                score_cyc[1],
            )
        ...
        if self.score_type == "BIC":
            BIC = (
                np.log(np.sum(RSS_ls) / self.n_samples + 1e-8)
                + np.sum(graph_batch) * self.bic_penalty / self.maxlen
            )
        elif self.score_type == "BIC_different_var":
            BIC = (
                np.sum(np.log(np.array(RSS_ls) / self.n_samples + 1e-8))
        ...
        if self.verbose:
            self._logger.info(
                "BIC: {}, cycness: {}, returned reward: {}".format(
                    BIC, cycness, final_score
                )
            )
