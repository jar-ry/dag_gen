...
def main(config_override):
    ...
    for key in config_override:
        print(key, config_override[key])
        config.__setattr__(key, config_override[key])
    time0 = time.time()
    print(config)
    ...
    _logger.info(
        "Configuration parameters: {}".format(vars(config))
    )  # Use vars to convert config to dict for logging

    if config.read_data:
        file_path = os.path.join(config.data_path, "data1.npy")
        ...
        training_set = DataGenerator_read_data(
            file_path, config.normalize, config.transpose
        )
    else:
    ...
    if config.lambda_flag_default:
        gtrue = None
        if training_set.true_graph:
            gtrue = training_set.true_graph.T
        sl, su, strue = BIC_lambdas(
            training_set.inputdata, None, None, gtrue, reg_type, score_type
        )
        ...
        _logger.info(
            "Transfomed sl: {}, su: {}, lambda2: {}, true: {}".format(
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        # Test tensor shape
        _logger.info(
            "Shape of actor.input: {}".format(sess.run(tf.shape(actor.input_)))
        )
        ...
        for i in range(1, config.nb_epoch + 1):
            ...
            input_batch = training_set.train_batch(
                actor.batch_size, actor.max_length, actor.input_dimension
            )
            ...
            if i == 1 or i % 500 == 0:
                _logger.info(
                    "[iter {}] reward_batch: {}, max_reward: {}, max_reward_batch: {}".format(
                        i, reward_batch, max_reward, max_reward_batch
                    )
                )
            ...
            if (i + 1) % lambda_iter_num == 0:
                ...
                _logger.info(
                    "[iter {}] lambda1 {}, upper {}, lambda2 {}, upper {}, score_min {}, cyc_min {}".format(
                        i + 1,
                        lambda1,
                        lambda1_upper,
                        lambda2,
                ...
                np.save(
                    "{}/solvd_graph_batch_pruned.npy".format(config.graph_dir),
                    graph_batch_pruned,
                )
        ...
        time1 = time.time()
        timing = time1 - time0
        with open(os.path.join(config.graph_dir, "timing.txt"), "w") as f:
            f.write(str(timing))
