...
def main(config_override):
    ...
    if config.read_data:
        ...
        training_set = DataGenerator_read_data(file_path, config.normalize, config.transpose)
    else:
    ...
    if config.lambda_flag_default:
        ...
        sl, su, strue = BIC_lambdas(training_set.inputdata, None, None, gtrue, reg_type, score_type)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for i in (range(1, config.nb_epoch + 1)):
            ...
            if (i+1) % lambda_iter_num == 0:
                ...
                np.save('{}/solvd_graph_batch.npy'.format(config.graph_dir), graph_batch)
                np.save('{}/solvd_graph_batch_pruned.npy'.format(config.graph_dir), graph_batch_pruned)
