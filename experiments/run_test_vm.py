...
def run_sam(data: array, output_path: str):
    ...
    time0 = time.time()
    obj = SAM(gpus=1, njobs=2, nruns=32, verbose=False)
    output = obj.predict(DataFrame(data).astype(float32))
    pred = to_numpy_matrix(output)
    time1 = time.time()
    print(time1, time0)
...
def run_rl_bic(data_path: str, output_path: str, shape: tuple):
    ...
    print("=================")
    from algorithms import causal_discovery_rl
...
def run_dag_gnn(data_path: str, output_path: str, shape: tuple):
    ...
    print("=================")
    from algorithms import dag_gnn
...
def run_gran_dag(data_path: str, output_path: str, shape: tuple):
    ...
    print("=================")
    from algorithms import gran_dag
        ...
        if mech_name == causal_mech_dir:
            ...
            for dataset in dataset_dirs:
                if dataset == test_name:
                    print("Running ", mech_name, test_name, algo_name)
                    # Configure paths
